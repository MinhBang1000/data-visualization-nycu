...
var sankey = d3
    ...
    .nodeWidth(20)
    .nodePadding(30)
    .size([daigramWidth + margin.left, daigramHeight + margin.top]);
...
const render = (graph) => {
    ...
    // Should be checked because it seems to be recursive
    ...
    const colorScales = {
        "buying": d3.scaleSequential(d3.interpolateBlues).domain([0,3]),
        "maintenance": d3.scaleSequential(d3.interpolateGreens).domain([0,3]),
        "doors": d3.scaleSequential(d3.interpolateGreys).domain([0,3]),
        "persons": d3.scaleSequential(d3.interpolateOranges).domain([0,2]),
        "luggage boot": d3.scaleSequential(d3.interpolatePurples).domain([0,2]),
        "safety": d3.scaleSequential(d3.interpolateReds).domain([0,2])
    }
    ...
    node
        ...
        .style('fill', function (d) {
            ...
            const colorScale = colorScales[name];            
            d.color = colorScale(colorScalesIndex[name][value])
            console.log(d.color);
...
d3.text('car.data').then(function (r) {
    console.log(r);
